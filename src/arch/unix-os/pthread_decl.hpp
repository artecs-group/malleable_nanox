...
namespace nanos {
   ...
   class PThread
   {
      ...
      public:
         // constructor
         PThread( ext::SMPProcessor * core ) : _core( core ), _pth( ), _stackSize( 0 ), _tid(-1) { }
         ...
         /**@ @**/
       inline int getRealCpuId() const {return sched_getcpu();}
       inline void setAffinity(int coreId){
	   cpu_set_t cpuset;
	   CPU_ZERO(&cpuset);
	   CPU_SET(coreId, &cpuset);
	   //puts("Pthread: Cambiando la afinidad");

	   if(_tid==-1)
	     pthread_setaffinity_np(_pth, sizeof(cpu_set_t), &cpuset);
	   else
	     sched_setaffinity(_tid, sizeof(cpu_set_t), &cpuset);
       }
