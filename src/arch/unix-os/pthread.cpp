...
void * os_bootthread ( void *arg )
{
   ...
   ((PThread*)self)->_tid=syscall(SYS_gettid);
...
{ 
   ...
   if(txt==NULL)
     ...
     int coreOffset = atoi(getenv("START_BINDING"));
     size_t firstCpu = binding.first();

     cpu_set_t cpuSet;

     CPU_ZERO(&cpuSet);
     CPU_SET(firstCpu + coreOffset, &cpuSet);
     pthread_setaffinity_np( _pth, sizeof(cpu_set_t), &cpuSet );

     verbose( "===> Affinity of thread " << firstCpu << " set to core " << firstCpu + coreOffset );     
   }
...
// /**@ @**/
// int PThread::getRealCpuId() const {  }
    
// void PThread::setAffinity(size_t cpusetsize, const cpu_set_t* cpuset) {
