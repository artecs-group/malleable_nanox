...
class SMPPlugin : public SMPBasePlugin
{
   ...
   std::vector<long long>       _frequency;
   /*@ @*/
   ...
   virtual std::pair<std::string, std::string> getBindingStrings() const;
    ...
    /**@ @**/
    virtual void setAffinity();
    virtual inline void setAffinity(int threadId, int coreId){
	_affinity[threadId] = coreId;
	if(_workers.size() > (unsigned)threadId)
	    _workers[threadId]->setAffinity(coreId);
    }
    virtual void setFrequency();
    virtual void setFrequency(int threadId, long long freq);
