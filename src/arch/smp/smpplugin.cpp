...
nanos::PE * smpProcessorFactory ( int id, int uid )
{
                 ...
                 , _asyncSMPTransfers( true )
   {
       ...
       _affinity=std::vector<int>(nProc, -1);
      _frequency=std::vector<long long>(nProc, -1);
       /*@   @*/
   }
   ...
   bool SMPPlugin::asyncTransfersEnabled() const {
    ...
    /**@ @**/
   void SMPPlugin::setAffinity()
   {
       for(unsigned i=0; i<_affinity.size(); i++){
	 if(_affinity[i]!=-1)
	   this->setAffinity(i, _affinity[i]);
       }
   }

  void SMPPlugin::setFrequency(){
    for(unsigned i=0; i<_frequency.size(); i++){
      if(_frequency[i] != -1)
	this->setFrequency(i, _frequency[i]);
    }
  }
      
  void SMPPlugin::setFrequency(int threadId, long long goalFreq){
    _frequency[threadId] = goalFreq;
    if(_workers.size() <= (unsigned)threadId) return;
    
    int coreId = (_affinity[threadId]!=-1) ? _affinity[threadId] : threadId;

    long long currFreq = cpufreq_get_freq_kernel(coreId);

    //fprintf(stderr, "Setting freq core %d: %Ld\n", coreId, goalFreq);
    if(currFreq == goalFreq) return;
    else if(currFreq < goalFreq){
      cpufreq_modify_policy_max(coreId, goalFreq);
      cpufreq_modify_policy_min(coreId, goalFreq);
      //cpufreq_set_frequency(coreId, goalFreq);
    } else {
      cpufreq_modify_policy_min(coreId, goalFreq);
      cpufreq_modify_policy_max(coreId, goalFreq);
      //cpufreq_set_frequency(coreId, goalFreq);
