...
#include "fgcsMalleableFreq_sched.hpp"
...
namespace nanos {
  namespace ext {
      ...
      FgcsMalleableFreq::FgcsMalleableFreq()  :
	  ...
	  if (getenv("MAX_BUDGET")!=NULL) _maxBudget=atof(getenv("MAX_BUDGET"));
	...
	/*@-----@*/
      ...
      void FgcsMalleableFreq::queue ( BaseThread *thread, WD &wd )
      {
	  //puts("QUEUE 1");
      ...
      void FgcsMalleableFreq::queue ( BaseThread ** threads, WD ** wds, size_t numElems )
      {
	  //puts("QUEUE 2");
      ...
      void FgcsMalleableFreq::atSuccessor   ( DependableObject &successor, DependableObject &predecessor )
      {
	  //puts("AT SUCCESSOR");	    
	  return;
      }
    
      WD *FgcsMalleableFreq::atSubmit ( BaseThread *thread, WD &newWD )
      {
	  //puts( "AT SUBMIT" );	    
	  queue( thread, newWD );
	  return 0;
      ...
      WD * FgcsMalleableFreq::atIdle ( BaseThread *thread, int numSteal )
      {
	  ...
	  const int maxThreadsPossible=std::min( _expControl.numCores, aux);
      ...
      WD * FgcsMalleableFreq::atPrefetch ( BaseThread *thread, WD &current )
      {
	  //puts("AT PREFETCH");
	  return atIdle(thread,false);
      }
    
      WD * FgcsMalleableFreq::atBeforeExit ( BaseThread *thread, WD &current, bool schedule )
      {
	  //puts("AT BEFORE EXIT");
      ...
      void FgcsMalleableFreq::atShutdown()
      {
	  //puts("AT SHUTDOWN");
	  ...
	  // for(int i=1; i<_expControl.numCores; i++){
	  // 	sem_trywait(&_expControl.sems[i]);
	  // }
	    
      }

      WD* FgcsMalleableFreq::atWakeUp ( BaseThread *thread, WD &wd )
      {
	  //puts("AT WAKE UP");
	  for(int i=0; i<_expControl.numCores; i++)
	      sem_post(&_expControl.sems[i]);
      ...
      //====================================================================
      void FgcsMalleableFreq::calculateNewFreq()
      {
      ...
      void FgcsMalleableFreq::changeFreq(int idx, long long freq) const
      {
      ...
      // void FgcsMalleableFreq::increaseFreq(int idx, long long freq) const
      // {
      //   cpufreq_modify_policy_max(idx, freq);
      //   cpufreq_modify_policy_min(idx, freq);
      // }
      
      // void FgcsMalleableFreq::decreaseFreq(int idx, long long freq) const
      // {
      //   cpufreq_modify_policy_min(idx, freq);
      //   cpufreq_modify_policy_max(idx, freq);
      // }
    ...
    //====================================================================
      void FgcsMalleableFreq::sleepThread(int threadId)
      {
	...
	/*@-----@*/
      ...
      void FgcsMalleableFreq::wakeUp_threads(int maxThreads)
      {	  
	  { LockBlock l(_expControl.threadStatus_lck);
...
DECLARE_PLUGIN("fgcsMalleableFreq",nanos::ext::FgcsMalleableFreqPlugin);
